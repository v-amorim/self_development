...
local function text_style()
	...
	if o.custom_header and o.custom_header ~= "" then
		...
		return style .. "\\xshad" .. shadow_x_offset .. "\\yshad" .. shadow_y_offset .. "}"
	end
end
...
local function get_kbinfo_lines()
	...
	for _, bind in pairs(bindings) do
		if
			...
			and (searched_text == nil or (bind.key .. bind.cmd):lower():find(searched_text, 1, true))
		then
...
local function append_display_sync(s)
	...
	if not display_timer.oneshot and (o.plot_vsync_ratio or o.plot_vsync_jitter) and o.use_ass then
		...
		append_property(
			s,
			"vsync-jitter",
			{ prefix = "VSync Jitter:", nl = vr and "" or o.nl, indent = vr and o.prefix_sep .. o.prefix_sep }
		)
	end
end
...
local function add_file(s, print_cache, print_tags)
	...
	append_property(
		s,
		"file-format",
		{ prefix = "Format/Protocol:", nl = fs and "" or o.nl, indent = fs and o.prefix_sep .. o.prefix_sep }
	)
	...
	if demuxer_cache + demuxer_secs > 0 then
		...
		append(
			s,
			format("%.1f", demuxer_secs),
			{ prefix = "(", suffix = " sec)", nl = "", no_prefix_markup = true, prefix_sep = "", indent = o.prefix_sep }
		)
	end
end
...
local function add_video(s)
	...
	-- 		s,
	-- 		"hwdec-current",
	-- 		{ prefix = "HW:", nl = "", indent = o.prefix_sep .. o.prefix_sep, no_prefix_markup = false, suffix = "" },
	-- 		{ no = true, [""] = true }
	-- 	)
	-- end
...
local function add_audio(s)
	...
	append(
		s,
		merge(r, ro, "format"),
		{ prefix = "Format:", nl = cc and "" or o.nl, indent = cc and o.prefix_sep .. o.prefix_sep }
	)
...
mp.add_key_binding(nil, "display-fps", function()
	process_key_binding(true)
end, { repeatable = true })

-- Toggling key binding
mp.add_key_binding(nil, "display-fps-toggle", function()
