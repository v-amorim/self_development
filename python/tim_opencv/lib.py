...
def create_figure(num_images, figsize):
...
def show_image(ax, img, is_grayscale=False, title=None):
...
def show_images(images, figsize=None):
    fig, axes = create_figure(len(images), figsize)
    for i, image in enumerate(images):
        ...
        is_grayscale = conversion == cv2.COLOR_BGR2GRAY
        title = image[1] if _1param and type(image[1]) == str else None

        show_image(axes.flat[i], img, is_grayscale=is_grayscale, title=title)
