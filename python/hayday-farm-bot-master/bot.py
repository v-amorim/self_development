...
class Bot:
    ...
    def get_target(self):
        screen_dim = {"left": 0, "top": 0, "width": 1920, "height": 1080}
        with mss.mss() as sct:
    ...
    def drag_operation(self, drag_start, matches, target):
        ...
        for x, y in path:
            if keyboard.is_pressed("q"):
                return
            pa.moveTo(x, y, duration=0.3, _pause=False)
            sleep(0.5)
    ...
    def combine_paths(self, p1, p2):
        result = [[x, y, w, h] for x, y, w, h in p1]
        result.extend([x, y, w, h] for x, y, w, h in p2)
        return result

    def translate_path(self, path, translation):
        tx, ty = translation
        return [[x + tx, y + ty, w, h] for x, y, w, h in path]
    ...
    def bot_loop(self):
        while True:
            ...
            if len(empty_fields) > 0:
                self.logger.log(f"Found {len(empty_fields)} empty fields... starting planting")
            ...
            if not self.silo_is_full:
                if len(grown_plants) > 0:
                    self.logger.log(f"Found {len(grown_plants)} grown plants, starting harvesting...")
